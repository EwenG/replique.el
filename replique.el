...
(defvar replique/active-repls nil)
(defvar replique/active-tooling-repl nil)
...
(defun replique/active-repl-by (&rest args)
  (apply 'replique/repls-or-repl-by '-first replique/active-repls args))

(defun replique/active-repls-by (&rest args)
  (apply 'replique/repls-or-repl-by '-filter replique/active-repls args))
...
(defun replique/replace-or-remove (old new list)
  (if (-contains? list old)
      (-replace old new list)
    (-remove (lambda (x) (equal x old)) list)))

(defun replique/close-repl (repl-props)
  (-let* (((&alist :buffer buffer) repl-props)
          (proc (get-buffer-process buffer)))
    (setq replique/repls (delete repl-props replique/repls))
    (when (and proc (process-live-p proc))
      (process-send-eof proc))))

(defun replique/update-active-tooling-repl (removed-repl)
  (-let* (((&alist :host host :port port) removed-repl)
          (repls (replique/repls-by :host host :port port :repl-type :tooling))
          (new-active-repl (-first (lambda (x) (not (eq x removed-repl))) repls)))
...
(defun replique/update-active-repls (removed-repl)
  (-let* (((&alist :host host :port port :repl-type repl-type) removed-repl)
          (repls (replique/repls-by :host host :port port :repl-type repl-type))
          (new-active-repl (-first (lambda (x) (not (eq x removed-repl))) repls)))
    (replique/replace-or-remove removed-repl new-active-repl replique/active-repls)))

(defun replique/remove-from-active-repls (host port)
  (let ((removed-repls (replique/active-repls-by :host host :port port)))
    (mapcar (lambda (repl)
              (setq replique/active-repls (delete repl replique/active-repls)))
            removed-repls)))
...
(defun replique/on-tooling-repl-close2
    (tooling-chan-src host port process event)
  (cond ((string= "deleted\n" event)
    ...
    (replique-async/<!
     chan (lambda (x)
            ...
            (let* ((repl-infos (replique-edn/read-string x))
                   ...
                   (directory (gethash :directory repl-infos))
...
(defun replique/delete-procs ()
  (let ((tooling-procs (replique/repls-by :repl-type :tooling)))
    (mapcar (-lambda ((&alist :proc p))
              (delete-process p)
